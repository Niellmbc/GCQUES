...
fetch(url+'/tbl_user/fldUserID/'+id).then(res=>res.json()).then(function(res){
	...
	document.getElementById('rAddrress').value=res[0].fldAddress;
	document.getElementById('rUname').value=res[0].fldUsername;
	document.getElementById('imgP').src=res[0].fldImg;
...
function readFile() {
	if (this.files && this.files[0]) {
		...
		FR.addEventListener("load", function (e) {
			document.getElementById("imgP").src= e.target.result;
...
let updateAdmin=()=>{
	...
	let data = {
		...
		fldAddress:document.getElementById('rAddrress').value,
		fldImg:base64img,
